...
#         """Receive initial pose from RViz; this is the also the return-to-start goal."""
#         self.end_goal = (msg.pose.position.x, msg.pose.position.y)
#         self.get_logger().info(f"Initial pose set to: {self.end_goal}")

#     def clicked_point_callback(self, msg: PointStamped):
#         """Receive clicked points from RViz as parking waypoints."""
#         point = (msg.point.x, msg.point.y)

#         # First click sets park1_goal, second click sets park2_goal. All others should be ignored.
...
#             # Don't publish yet; we'll publish after backing up from park1
...
#                 # Check if near initial pose to declare mission done
#                 init_x, init_y = self.end_goal
#                 if np.hypot(car_x - init_x, car_y - init_y) <= self.goal_threshold:
#                     self.state = State.DONE
#                     self.get_logger().info("DONE: returned to initial pose")
#             else:
